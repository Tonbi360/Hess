...
type View = 'menu' | 'vsplayer' | 'join' | 'vsai' | 'selfplay' | 'playground';
...
export function LobbyPage({ state, actions }: LobbyPageProps) {
  ...
  const [hessTaps, setHessTaps] = useState(0);
    ...
    <div className="min-h-[100dvh] bg-background text-foreground flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-2">
          <h1 
            onClick={() => setHessTaps(prev => prev + 1)}
            className="font-serif text-6xl text-primary tracking-widest cursor-pointer select-none active:scale-95 transition-transform"
            title="Tap 4 times to unlock Admin mode"
          >
            HESS
          </h1>
          <p className="text-muted-foreground text-sm tracking-wider">
            {hessTaps >= 4 ? <span className="text-amber-400 font-mono text-xs uppercase tracking-widest">[ Admin Unlocked ]</span> : 'The War on the Board'}
          </p>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="menu" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
              ...
              {hessTaps >= 4 && (
                <button onClick={() => setView('selfplay')}
                  className="w-full py-4 rounded-xl border border-amber-500/40 text-amber-300 font-serif tracking-widest text-lg hover:border-amber-400 hover:text-amber-200 transition-colors text-left px-6 flex items-center justify-between bg-amber-950/20">
                  <span>Hess vs Hess <span className="text-amber-400/60 font-sans text-xs font-normal">admin</span></span>
                  <span className="text-amber-400/40 text-base">⟶</span>
                </button>
              )}
              <button onClick={() => setView('playground')}
                className="w-full py-4 rounded-xl border border-primary/20 bg-primary/5 text-foreground font-serif tracking-widest text-lg hover:bg-primary/10 hover:border-primary/40 transition-colors text-left px-6 flex items-center justify-between">
                <span>Rules Playground <span className="text-primary/70 font-sans text-xs font-normal">sandbox</span></span>
                <span className="text-primary/40 text-base">⟶</span>
              </button>
            </motion.div>
          )}
          ...
          {/* ── Rules Playground ── */}
          {view === 'playground' && (
            <RulesPlayground onBack={() => setView('menu')} />
          )}
