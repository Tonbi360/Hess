...
const PIECE_SACRIFICE_VALUES: Record<string, number> = {
  PAWN: 1, KNIGHT: 3, BISHOP: 3, JESTER: 4, ROOK: 5, QUEEN: 9, KING: 100,
};

function pickBestSacrificeTarget(state: GameState, targets: number[]): number {
  if (targets.length === 0) return -1;
  let bestSq = targets[0];
  let minVal = 999;
  for (const sq of targets) {
    const p = state.board[sq];
    const val = p ? (PIECE_SACRIFICE_VALUES[p.type] ?? 1) : 1;
    if (val < minVal) {
      minVal = val;
      bestSq = sq;
    }
...
export function handleMakeMove(room: Room, color: Color, from: number, to: number): MakeMoveResult {
  ...
  if (result.state.phase === 'JESTER_SACRIFICE' &&
    ...
    if (targets.length > 0) {
      const bestTarget = pickBestSacrificeTarget(result.state, targets);
      room.gameState = applyJesterSacrifice(result.state, bestTarget);
...
export function computeAndApplyAiMove(room: Room): GameState | null {
  if (!room.isAIGame) return null;

  // 1. Handle JESTER_SACRIFICE phase if AI needs to sacrifice
  if (room.gameState.phase === 'JESTER_SACRIFICE') {
    if (room.gameState.jesterSacrificeCtx?.sacrificingColor === 'BLACK') {
      const targets = getValidSacrificeTargets(room.gameState);
      if (targets.length > 0) {
        const bestTarget = pickBestSacrificeTarget(room.gameState, targets);
        room.gameState = applyJesterSacrifice(room.gameState, bestTarget);
        if (room.gameState.phase === 'GAME_OVER') triggerAiTraining(room);
  ...
  // 2. Handle SETUP phase
  if (room.gameState.phase === 'SETUP_BLACK') {
    room.gameState = confirmSetup(room.gameState);
    return room.gameState;
  }
  ...
  if (!move) {
    ...
    const isCheck = kingSq !== -1 && isSquareAttackedBy(room.gameState, kingSq, 'WHITE');
    if (isCheck) {
      room.gameState = {
        ...room.gameState,
        phase: 'GAME_OVER',
