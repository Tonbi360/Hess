...
export function createInitialState(): GameState {
  return {
    ...
    capturedByBlack: [],
    moveHistory: [],
    whiteQueenRaidMoves: 0,
    whiteQueenRaidLaunchSq: null,
    blackQueenRaidMoves: 0,
    blackQueenRaidLaunchSq: null,
  };
}
...
function isQueenRaidUnlocked(board: Board, color: Color): boolean {
  for (let sq = 0; sq < 64; sq++) {
    const p = board[sq];
    if (p && p.color === color && p.type !== 'KING' && p.type !== 'QUEEN') {
      return false;
    }
  }
  return true;
...
function getQueenMoves(board: Board, sq: number, color: Color): number[] {
  ...
  const raidUnlocked = isQueenRaidUnlocked(board, color);
  ...
  for (const [dr, dc] of directions) {
    ...
    while (true) {
      ...
      if (!raidUnlocked && !territory(target)) break;
      const piece = board[target];
...
export function getValidSwapTargets(state: GameState, color: Color): number[] {
  ...
  for (let sq = 0; sq < 64; sq++) {
    const p = state.board[sq];
    // King Swap ONLY works on friendly Pawns
    if (!p || p.color !== color || p.type !== 'PAWN') continue;
...
export function applyMove(state: GameState, from: number, to: number): MoveResult {
  ...
  let whiteQueenRaidMoves = state.whiteQueenRaidMoves || 0;
  let whiteQueenRaidLaunchSq = state.whiteQueenRaidLaunchSq ?? null;
  let blackQueenRaidMoves = state.blackQueenRaidMoves || 0;
  let blackQueenRaidLaunchSq = state.blackQueenRaidLaunchSq ?? null;

  if (movingPiece.type === 'QUEEN') {
    const isWhite = movingPiece.color === 'WHITE';
    const isEnemyTerritory = isWhite ? !isWhiteTerritory(to) : !isBlackTerritory(to);
    const isHomeTerritory = isWhite ? isWhiteTerritory(from) : isBlackTerritory(from);

    if (isWhite) {
      if (isHomeTerritory && isEnemyTerritory) {
        whiteQueenRaidLaunchSq = from;
        whiteQueenRaidMoves = 1;
      ...
      if (whiteQueenRaidMoves >= 5 && whiteQueenRaidLaunchSq !== null) {
        board[whiteQueenRaidLaunchSq] = board[to];
        board[to] = null;
        whiteQueenRaidMoves = 0;
        whiteQueenRaidLaunchSq = null;
      }
      ...
      if (isHomeTerritory && isEnemyTerritory) {
        blackQueenRaidLaunchSq = from;
        blackQueenRaidMoves = 1;
      } else if (isEnemyTerritory) {
        blackQueenRaidMoves += 1;
      } else {
        blackQueenRaidMoves = 0;
      ...
      if (blackQueenRaidMoves >= 5 && blackQueenRaidLaunchSq !== null) {
        board[blackQueenRaidLaunchSq] = board[to];
        board[to] = null;
        blackQueenRaidMoves = 0;
        blackQueenRaidLaunchSq = null;
      }
  ...
  const nextState: GameState = {
    ...
    whiteQueenRaidMoves, whiteQueenRaidLaunchSq,
    blackQueenRaidMoves, blackQueenRaidLaunchSq,
  };
...
export function getValidSacrificeTargets(state: GameState): number[] {
  ...
  const { sacrificingColor } = state.jesterSacrificeCtx;
  const targets: number[] = [];
  for (let sq = 0; sq < 64; sq++) {
    const p = state.board[sq];
